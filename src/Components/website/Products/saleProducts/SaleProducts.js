...
export default function SaleProducts(props) {
  ...
  return (
    <NavLink
      to={`/product/${props.id}`}
      className={`col-lg-${props.col} col-md-6 col-12 mt-3`}
    >
      <div className="border rounded p-3 h-100 d-flex flex-column justify-content-between mx-2">
        <div className="text-truncate" style={{ color: "gray" }}>
          {props.title}
        </div>
        ...
        <div className="px-5 py-5 position-relative">
          {props.sale && (
            <div
              className="position-absolute top-0 start-0 bg-primary rounded-circle text-white text-uppercase d-inline-block text-center"
